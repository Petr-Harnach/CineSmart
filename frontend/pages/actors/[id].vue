<template>
  <div class="bg-gray-100 dark:bg-gray-900 pb-12">
    ...
    <!-- MAIN CONTENT -->
    <div v-else-if="actor" class="container mx-auto px-4 mt-8">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 md:p-8 flex flex-col md:flex-row gap-8">
        ...
        <!-- Right: Details -->
        <div class="flex-grow">
          <h1 class="text-4xl md:text-5xl font-black text-gray-900 dark:text-white tracking-tight">{{ actor.name }}</h1>
          
          <div v-if="actor.bio" class="mt-4 prose prose-sm dark:prose-invert text-gray-600 dark:text-gray-300 max-h-48 overflow-y-auto pr-2">
            <p>{{ actor.bio }}</p>
          </div>
          ...
          <div class="mt-6 grid grid-cols-2 gap-4 text-sm border-t border-gray-200 dark:border-gray-700 pt-6">
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                <span class="block text-xs text-gray-500">Místo</span>
                <span class="font-bold text-gray-800 dark:text-white">N/A</span>
              </div>
            </div>
            <div class="flex items-start gap-3">
...
const movies = computed(() => actor.value?.movies.filter(m => m.type === 'movie') || []);
const series = computed(() => actor.value?.movies.filter(m => m.type === 'series') || []);
...
const mostFamousFor = computed(() => {
    if (!actor.value?.movies || actor.value.movies.length === 0) return 'N/A';
    const allGenres = actor.value.movies.flatMap(m => m.genres?.map(g => g.name) || []);
