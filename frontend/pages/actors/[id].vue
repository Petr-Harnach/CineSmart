<template>
  ...
  <div v-else-if="actor" class="max-w-6xl mx-auto mt-10 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
    <!-- Actor Header Card -->
    <div class="flex flex-col md:flex-row gap-6 p-6">
      <div class="flex-shrink-0 w-36 md:w-48">
        <img v-if="actor.photo" :src="actor.photo" :alt="actor.name" class="w-full h-auto rounded-lg shadow-md">
      ...
      <div class="flex-grow">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-sm text-gray-600 dark:text-gray-300">
          <div>
            <span class="font-semibold text-gray-900 dark:text-gray-200">Role:</span> Herec
          </div>
          <div v-if="actor.birth_date">
            <span class="font-semibold text-gray-900 dark:text-gray-200">Narozen:</span> {{ new Date(actor.birth_date).toLocaleDateString() }}
          </div>
          <div v-if="actor.birth_place">
            <span class="font-semibold text-gray-900 dark:text-gray-200">Místo narození:</span> {{ actor.birth_place }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <p v-else class="text-center text-gray-500 italic py-8">V naší databázi nebyly nalezeny žádné {{ selectedFilmographyType === 'movie' ? 'filmy' : 'seriály' }} pro tohoto herce.</p>
    </div>
  </div>
</template>
