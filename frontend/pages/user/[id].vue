<template>
  <div v-if="loading" class="text-center text-gray-500 py-12">
    <p>Načítám profil uživatele...</p>
  </div>
  <div v-else-if="error" class="text-center text-red-500 py-12">
    <p>Nepodařilo se načíst profil uživatele: {{ error.message }}</p>
  </div>
  <div v-else-if="user" class="max-w-4xl mx-auto mt-10 p-4">
    ...
    <div class="flex flex-col items-center md:flex-row md:items-start gap-8">
      ...
      <div class="text-center md:text-left">
        ...
        <p v-else class="mt-2 text-gray-500 italic">Tento uživatel zatím nenapsal žádné bio.</p>
      </div>
    </div>
    ...
    <!-- User's Reviews -->
    <div class="mb-12">
      <h2 class="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-100">Recenze od {{ user.username }}</h2>
      ...
      <p v-else class="text-center text-gray-500 italic py-8">Tento uživatel zatím nenapsal žádné recenze.</p>
    </div>
    ...
    <div>
      <h2 class="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-100">Veřejné kolekce</h2>
          ...
          <details class="group">
            <summary class="flex justify-between items-center p-6 cursor-pointer list-none">
              ...
              <div class="flex items-center">
                <span class="text-sm text-gray-500 mr-2">{{ collection.items.length }} filmů</span>
            ...
            <div class="p-6 pt-0 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
              ...
              <p v-else class="text-gray-500 italic text-sm mt-2">Tato kolekce je prázdná.</p>
            </div>
          </details>
        </div>
      </div>
      <p v-else class="text-center text-gray-500 italic py-8">Tento uživatel nemá žádné veřejné kolekce.</p>
    </div>
  </div>
</template>
