<template>
  <div class="max-w-6xl mx-auto mt-10 p-4">
    <div class="flex justify-between items-center mb-8">
      ...
      >
        {{ showCreateForm ? 'Zrušit' : '+ Vytvořit kolekci' }}
      </button>
    </div>
    ...
    <div v-if="showCreateForm" class="mb-12 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
      <h2 class="text-xl font-bold mb-4 dark:text-gray-100">Nová kolekce</h2>
      <form @submit.prevent="handleCreate">
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Název</label>
        ...
        <div class="mb-6 flex items-center">
          ...
          <label for="is_public" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">Zveřejnit tuto kolekci</label>
        </div>
        <button type="submit" :disabled="isCreating" class="w-full py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:bg-green-400">
          {{ isCreating ? 'Vytvářím...' : 'Vytvořit kolekci' }}
        </button>
      </form>
    </div>

    <!-- Collections List -->
    <div v-if="loading" class="text-center text-gray-500 py-12">Načítám kolekce...</div>
    <div v-else-if="collections.length === 0" class="text-center text-gray-500 py-12 italic">
      Nebyly nalezeny žádné kolekce. Proč nevytvořit první?
    </div>
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <div class="flex justify-between items-start mb-2">
          ...
          <span v-if="!collection.is_public" class="px-2 py-1 bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs rounded">Soukromá</span>
        </div>
        <p class="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2 h-10">{{ collection.description || 'Popis nebyl zadán.' }}</p>
        <div class="flex justify-between items-center mt-auto">
          <span class="text-xs text-gray-500">Od {{ collection.user.username }}</span>
          
          <div class="flex items-center gap-3">
            <span class="text-xs font-semibold text-blue-600 dark:text-blue-400">{{ collection.items.length }} položek</span>
            <button 
