  ...
  <div v-else-if="director" class="max-w-6xl mx-auto mt-10 p-4">
    <div class="flex flex-col md:flex-row gap-8">
      <div class="md:w-1/3 text-center">
        <img v-if="director.photo" :src="director.photo" :alt="director.name" class="w-full h-auto rounded-lg shadow-lg mx-auto">
    ...
    <div>
      ...
      <div v-if="director.movies && director.movies.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div 
          ...
          @click="$emit('show-detail', movie.id)"
        >
...
onMounted(() => {
  fetchDirector(props.directorId);
});

// Znovu načíst data, pokud se změní directorId
watch(() => props.directorId, (newId) => {
  fetchDirector(newId);
});
</script>
